...
void NovaLevelAudioProcessor::processBlock (juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midiMessages)
{
    ...
    outputPeakLevel.store    (levelDSP.getOutputPeak());
    gainReductionLevel.store (levelDSP.getGainReductionDb());
    outputIsHot.store        (levelDSP.isOutputHot());
}

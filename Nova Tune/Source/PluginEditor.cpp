...
NovaTuneAudioProcessorEditor::NovaTuneAudioProcessorEditor (NovaTuneAudioProcessor& processor)
...
void NovaTuneAudioProcessorEditor::paint (juce::Graphics& graphics)
{
...
void NovaTuneAudioProcessorEditor::timerCallback()
{
    if (webView == nullptr || ! webView->isVisible())
        return;

    auto serialiseSpectrum = [] (const std::array<std::atomic<float>, NovaTuneAudioProcessor::spectrumBins>& spectrum)
    {
        juce::String output;
        output << "[";
...
juce::WebBrowserComponent::Options NovaTuneAudioProcessorEditor::createWebOptions (NovaTuneAudioProcessorEditor& editor)
{
...
std::optional<juce::WebBrowserComponent::Resource> NovaTuneAudioProcessorEditor::getResource (const juce::String& url)
{
    ...
    if (lowerUrl.contains ("index.html"))
        return makeResource (nova_tune_BinaryData::index_html,
                     nova_tune_BinaryData::index_htmlSize,
                             "text/html");

    if (lowerUrl.contains ("n_logo.png"))
        return makeResource (nova_tune_BinaryData::n_logo_png,
                     nova_tune_BinaryData::n_logo_pngSize,
                             "image/png");

    if (lowerUrl.contains ("js/index.js"))
        return makeResource (nova_tune_BinaryData::index_js,
                     nova_tune_BinaryData::index_jsSize,
                             "text/javascript");

    if (lowerUrl.contains ("js/fallback-boot.js"))
        return makeResource (nova_tune_BinaryData::fallbackboot_js,
                     nova_tune_BinaryData::fallbackboot_jsSize,
                             "text/javascript");

    if (lowerUrl.contains ("js/juce/index.js"))
        return makeResource (nova_tune_BinaryData::index_js2,
                     nova_tune_BinaryData::index_js2Size,
                             "text/javascript");

    if (lowerUrl.contains ("js/juce/check_native_interop.js"))
        return makeResource (nova_tune_BinaryData::check_native_interop_js,
                     nova_tune_BinaryData::check_native_interop_jsSize,
                             "text/javascript");

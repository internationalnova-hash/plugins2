...
NovaAutotuneAudioProcessorEditor::NovaAutotuneAudioProcessorEditor (NovaAutotuneAudioProcessor& p)
    : AudioProcessorEditor (&p), processorRef (p)
{
    ...
    retuneSpeedAttachment = std::make_unique<juce::WebSliderParameterAttachment> (*processorRef.apvts.getParameter ("retuneSpeed"), retuneSpeedRelay, nullptr);
...
NovaAutotuneAudioProcessorEditor::~NovaAutotuneAudioProcessorEditor()
{
    stopTimer();
}

void NovaAutotuneAudioProcessorEditor::paint (juce::Graphics& g)
{
    g.fillAll (juce::Colour::fromRGB (8, 17, 29));
}

void NovaAutotuneAudioProcessorEditor::resized()
{
    if (webView != nullptr)
        webView->setBounds (getLocalBounds());
}

void NovaAutotuneAudioProcessorEditor::timerCallback()
{
...
juce::WebBrowserComponent::Options NovaAutotuneAudioProcessorEditor::createWebOptions (NovaAutotuneAudioProcessorEditor& editor)
{
    ...
    options = options.withNativeIntegrationEnabled()
                     ...
                     .withOptionsFrom (editor.retuneSpeedRelay)
...
std::optional<juce::WebBrowserComponent::Resource> NovaAutotuneAudioProcessorEditor::getResource (const juce::String& url)
{
    ...
    if (lowerUrl.contains ("index.html"))
        return makeResource (nova_autotune_BinaryData::index_html,
                             nova_autotune_BinaryData::index_htmlSize,
                             "text/html");

    if (lowerUrl.contains ("n_logo.png"))
        return makeResource (nova_autotune_BinaryData::n_logo_png,
                             nova_autotune_BinaryData::n_logo_pngSize,
                             "image/png");

    if (lowerUrl.contains ("js/index.js"))
        return makeResource (nova_autotune_BinaryData::index_js,
                             nova_autotune_BinaryData::index_jsSize,
                             "text/javascript");

    if (lowerUrl.contains ("js/fallback-boot.js"))
        return makeResource (nova_autotune_BinaryData::fallbackboot_js,
                             nova_autotune_BinaryData::fallbackboot_jsSize,
                             "text/javascript");

    if (lowerUrl.contains ("js/juce/index.js"))
        return makeResource (nova_autotune_BinaryData::index_js2,
                             nova_autotune_BinaryData::index_js2Size,
                             "text/javascript");

    if (lowerUrl.contains ("js/juce/check_native_interop.js"))
        return makeResource (nova_autotune_BinaryData::check_native_interop_js,
                             nova_autotune_BinaryData::check_native_interop_jsSize,
                             "text/javascript");

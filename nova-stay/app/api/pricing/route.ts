...
// GET — fetch current default pricing settings
export async function GET(req: NextRequest) {
  ...
  if (!propertyId) return NextResponse.json({ defaultPrice: 250, minNights: 1 });
...
// POST — upsert default pricing settings
export async function POST(req: NextRequest) {
  ...
  const { defaultPrice, minNights } = await req.json();
  const propertyId = auth?.propertyId;
  if (!propertyId) return NextResponse.json({ error: "No property" }, { status: 400 });

  await sql`
    INSERT INTO property_pricing (property_id, default_price_per_night, min_nights, updated_at)
    VALUES (${propertyId}, ${Number(defaultPrice) || 250}, ${Number(minNights) || 1}, now())

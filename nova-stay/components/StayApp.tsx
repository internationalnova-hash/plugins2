...
export default function StayApp({ propertySlug }: { propertySlug?: string }) {
  ...
  useEffect(() => {
    ...
    // When this app is rendered for a specific property (path-based
    // multi-tenant route), record the active property slug so API calls
    // made elsewhere (host dashboard, etc.) scope to it.
    if (propertySlug) {
      try {
        localStorage.setItem("nova_active_property_slug", propertySlug);
      } catch { /* ignore */ }
    }
    ...
    const propertyUrl = propertySlug ? `/api/property?slug=${encodeURIComponent(propertySlug)}` : "/api/property";
    fetch(propertyUrl)

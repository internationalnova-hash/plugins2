...
NovaSilkAudioProcessorEditor::NovaSilkAudioProcessorEditor (NovaSilkAudioProcessor& processor)
    : AudioProcessorEditor (&processor), processorRef (processor)
{
    webView = std::make_unique<SinglePageBrowser> (createWebOptions (*this));

    smoothAttachment = std::make_unique<juce::WebSliderParameterAttachment> (*processorRef.apvts.getParameter ("smooth"), smoothRelay, nullptr);
    focusAttachment = std::make_unique<juce::WebSliderParameterAttachment> (*processorRef.apvts.getParameter ("focus"), focusRelay, nullptr);
    airAttachment = std::make_unique<juce::WebSliderParameterAttachment> (*processorRef.apvts.getParameter ("air_preserve"), airRelay, nullptr);
    bodyAttachment = std::make_unique<juce::WebSliderParameterAttachment> (*processorRef.apvts.getParameter ("body"), bodyRelay, nullptr);
...
NovaSilkAudioProcessorEditor::~NovaSilkAudioProcessorEditor()
{
    stopTimer();
}

void NovaSilkAudioProcessorEditor::paint (juce::Graphics& graphics)
{
    graphics.fillAll (juce::Colour::fromRGB (234, 232, 235));
}

void NovaSilkAudioProcessorEditor::resized()
{
    if (webView != nullptr)
        webView->setBounds (getLocalBounds());
}

void NovaSilkAudioProcessorEditor::timerCallback()
{
    if (webView == nullptr || ! webView->isVisible())
        return;

    auto serialiseSpectrum = [] (const std::array<std::atomic<float>, NovaSilkAudioProcessor::spectrumBins>& spectrum)
    {
        juce::String output;
        output << "[";
    ...
    const auto script = "if (window.updateSilkSpectrum) { window.updateSilkSpectrum("
...
juce::WebBrowserComponent::Options NovaSilkAudioProcessorEditor::createWebOptions (NovaSilkAudioProcessorEditor& editor)
{
    ...
    options = options.withNativeIntegrationEnabled()
                     ...
                     .withOptionsFrom (editor.smoothRelay)
                     .withOptionsFrom (editor.focusRelay)
...
std::optional<juce::WebBrowserComponent::Resource> NovaSilkAudioProcessorEditor::getResource (const juce::String& url)
{
    ...
    if (lowerUrl.contains ("index.html"))
        return makeResource (nova_silk_BinaryData::index_html,
                             nova_silk_BinaryData::index_htmlSize,
                             "text/html");

    if (lowerUrl.contains ("n_logo.png"))
        return makeResource (nova_silk_BinaryData::n_logo_png,
